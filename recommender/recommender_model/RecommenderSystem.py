...
class MemoryBased():
    def recommend(self,user_id, current_state, session_id):

        current_state = float(current_state)
        user_id = int(user_id)

        user_df = pd.read_csv('./DataFrames/user_df.csv',index_col=0)
        neutral_state = user_df['score'].loc[user_id]

        target_state = neutral_state + (neutral_state - current_state)
        
        rate_df = pd.read_csv('DataFrames/rate_df.csv', index_col=0)
        ...
        Log_MemoryBased_Recommend = pd.read_csv("./DataFrames/Logs/Log_MemoryBased_Recommend.csv", index_col=0)
...
class CollaborativeFiltering():
    def recommend(user_id, music_id, session_id):
        ...
        similarity_users = pd.DataFrame(similarity_users, columns=rate_df.index, index = rate_df.index)
        similar_user = similarity_users.iloc[user_id].sort_values(ascending=False).drop(index=user_id).head(1)
        ...
        Log_CollaborativeFiltering_Recommend = pd.read_csv("./DataFrames/Logs/Log_CollaborativeFiltering_Recommend.csv", index_col=0)
        input_log_df = pd.DataFrame(np.array([[session_id, user_id, music_id, similar_user_id, similar_user_similarity, similar_user_rate]]), columns=Log_CollaborativeFiltering_Recommend.columns)
        Log_CollaborativeFiltering_Recommend = pd.concat((Log_CollaborativeFiltering_Recommend, input_log_df))
...
class Updater():
    ...
    def fit(self,music_dataframe, sensor_dataframe):
        ...
        for i in scores_df.iloc:
            self.update_rate_df(i['session_id'], i['user_id'], i['music_id'], i['score'])
            self.update_neutral_state(i['session_id'], i['user_id'], i['music_id'], i['score'])
    ...
    def update_neutral_state(self, session_id, user_id, music_id, value):
        ...
        Log_Updater_Score = pd.read_csv("./DataFrames/Logs/Log_Updater_Score.csv", index_col=0)
        input_log_df = pd.DataFrame(np.array([[session_id, user_id, music_id, previous_value, value, updated_value]]), columns=Log_Updater_Score.columns)
    ...
    def update_rate_df(self, session_id, user_id, music_id, value):
        ...
        Log_Updater_Update = pd.read_csv("./DataFrames/Logs/Log_Updater_Update.csv", index_col=0)
        input_log_df = pd.DataFrame(np.array([[session_id, user_id, music_id, previous_value, value, updated_value]]), columns=Log_Updater_Update.columns)
    ...
    def get_scores(self, music_dataframe, sensor_dataframe):
        ...
        for i in musics:
            try:
                music_chunk = cleaned_music_df.iloc[list(group.groups[i])].set_index('date_time')
                sensor_chunk = cleaned_sensor_df.iloc[list(group.groups[i])].set_index('date_time')
                ...
                if(music_chunk.shape[0] > 120 and sensor_chunk.shape[0] > 120):
                    ...
                    driving_scores.append((i,cleaned_music_df.loc[0].user_id,cleaned_music_df.loc[0].session_id, driving_score))
            except:
                print('error occured at: ' + str(i))
        df_score = pd.DataFrame(driving_scores, columns=['music_id', 'user_id', 'session_id', 'score'])
        return df_score
